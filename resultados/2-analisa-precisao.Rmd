...
```{r}
cor <- cor(reclamacoes_l$polaridade_normalizada, reclamacoes_l$palavras)
cor
...
reclamacoes_l %>% 
    ggplot(aes(x = insatisfacao, y = comprimento_reclamacao, group = insatisfacao)) + 
    geom_jitter(alpha = .5)  +
    # geom_boxplo() + 
    facet_wrap(~ lexico)
```
...
```{r}
reg2 <-lm(polaridade_normalizada ~ polaridade, reclamacoes_l)
summary(reg2)
```
